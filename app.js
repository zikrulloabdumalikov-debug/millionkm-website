  ...
  onSnapshot(qCars, (snapshot) => {
    ...
    snapshot.forEach((carDoc) => {
      ...
      card.innerHTML = `
        ...
        <p class="muted" style="margin: 8px 0 0 0;">Oxirgi servis yozuvi: <span class="last-log">—</span></p>
      ...
      // Oxirgi servis yozuvi (serviceLogs)
      (async () => {
        const lastEl = card.querySelector(".last-log");
        const last = await getLatestServiceLog(id, uid);
        if (!lastEl) return;
        if (!last) { lastEl.textContent = "yo‘q"; return; }
        const dt = last.createdAt?.toDate ? last.createdAt.toDate() : new Date(last.createdAt);
        const ds = isNaN(dt.getTime()) ? "" : dt.toLocaleDateString();
        lastEl.textContent = `${safeText(last.type)} • ${Number(last.km||0).toLocaleString()} km${ds ? " • " + ds : ""}`;
      })();
    ...
    if (sel) {
      ...
      snapshot.forEach((carDoc) => {
        ...
        const opt = document.createElement("option");
        opt.value = carDoc.id;
        opt.textContent = `${c.model || "Mashina"} (${Number(c.year)||""})`;
        sel.appendChild(opt);
      });
      if (!sel.children.length) {
        const opt = document.createElement("option");
        opt.value = "";
        opt.textContent = "Avval mashina qo‘shing";
        sel.appendChild(opt);
      }
    }
...
function bindChat() {
  ...
  async async function sendChatMessage() {
    ...
    const chatId = user ? user.uid : getPersistedChatId();
...
// =====================
// 15A) UI: MOBILE NAV, MODALS, GLOBAL SEARCH
// =====================
...
// Chat ID (persist)
const CHAT_ID_KEY = "mk_chat_id_v1";
function getPersistedChatId() {
  const v = localStorage.getItem(CHAT_ID_KEY);
  if (v) return v;
  const gen = "guest_" + Date.now();
  localStorage.setItem(CHAT_ID_KEY, gen);
  return gen;
}
function setPersistedChatId(id) {
  if (!id) return;
  localStorage.setItem(CHAT_ID_KEY, id);
}

function bindPackageModal() {
  const oneList = $("packageOneList");
  const yearList = $("packageYearList");
  if (oneList && Array.isArray(ONE_TIME_LIST)) {
    oneList.innerHTML = ONE_TIME_LIST.map((x) => `<li>${safeText(x)}</li>`).join("");
  }
  if (yearList && Array.isArray(YEAR_LIST)) {
    yearList.innerHTML = YEAR_LIST.map((x) => `<li>${safeText(x)}</li>`).join("");
  }

  document.querySelectorAll('[data-open="packageModal"]').forEach((b) => {
    b.addEventListener("click", () => openModal("packageModal"));
  });

  $("packageOrderBtn")?.addEventListener("click", () => {
    closeModal("packageModal");
    // foydalanuvchini buyurtmaga olib borish
    const orderModal = $("serviceModal");
    if (orderModal) openModal("serviceModal");
  });
}
  ...
  Object.keys(carData || {}).forEach((brand) => {
    ...
    Object.keys(models).forEach((model) => {
      ...
      items.push({
        ...
        model,
        title: `${brand} — ${model}`,
        text: `${brand} ${model} ${d.maxAge ?? ""} ${d.maxKm ?? ""} ${d.oneTimePrice ?? ""}`,
      });
    });
  });

  // statik FAQ/knowledge (SEO + qidiruv uchun)
  const faq = [
    { title: "Moy qachon almashtiriladi?", text: "Ko‘pchilik mashinada 7000–10000 km oralig‘ida. Sharoitga qarab farq qiladi.", tag: "FAQ" },
    { title: "Tormoz tekshiruvlari", text: "Kolodka, disk, suyuqlik va shlanglar muntazam tekshiriladi.", tag: "FAQ" },
    { title: "Diagnostika nima beradi?", text: "Kompyuter diagnostika xatoliklarni aniqlash, sensorlar va tizimlarni tekshirish.", tag: "FAQ" },
  ];
  faq.forEach((f) => items.push({ type:"faq", title:f.title, text:f.text, tag:f.tag }));
...
function renderSearchResults(container, results) {
  if (!container) return;
  if (!results.length) {
    container.innerHTML = `<p class="muted" style="padding:10px 2px;">Hech narsa topilmadi.</p>`;
    return;
  }
  container.innerHTML = results.slice(0, 12).map((r) => {
    const tag = r.type === "model" ? "Model" : (r.tag || "Ma’lumot");
    const subtitle = r.type === "model" ? `${safeText(r.brand)} • ${safeText(r.model)}` : safeText(r.text);
    return `
      <div class="search-result-card" data-type="${r.type}" data-brand="${safeText(r.brand||"")}" data-model="${safeText(r.model||"")}">
        <div class="tag">${tag}</div>
        <h4 style="margin:0 0 6px 0;">${safeText(r.title)}</h4>
        <p class="muted" style="margin:0;">${safeText(subtitle)}</p>
      </div>
    `;
  }).join("");

  // model card bosilsa brend modal ochilsin
  container.querySelectorAll(".search-result-card[data-type='model']").forEach((el) => {
...
function bindGlobalSearch() {
  ...
  const modalInput = $("globalSearchModalInput");
  const modalBtn = $("globalSearchModalBtn");
  ...
  const doSearch = (q) => {
    ...
    const items = idx();
    const res = query
      ? items.filter((it) => normalizeText(it.title + " " + it.text).includes(query))
      : [];
    renderSearchResults(resultsBox, res);
  };
  ...
  btn.addEventListener("click", () => openWith(input.value));
  input.addEventListener("keydown", (e) => {
    if (e.key === "Enter") openWith(input.value);
  });

  $("globalSearchModalBtn")?.addEventListener("click", () => doSearch(modalInput?.value || ""));
  $("globalSearchModalInput")?.addEventListener("keydown", (e) => {
    if (e.key === "Enter") doSearch(modalInput?.value || "");
  });
}

// =====================
// 15B) GARAGE: qidiruv + servis yozuvlari
// =====================
async function getLatestServiceLog(carId, uid) {
  try {
    const ql = query(
    ...
    console.warn("serviceLogs o‘qishda xato:", e);
    return null;
  }
}

function bindGarageSearch() {
  const s = $("garageSearch");
  const box = $("garage");
  if (!s || !box) return;

  s.addEventListener("input", () => {
    const q = normalizeText(s.value);
    box.querySelectorAll(".cabinet-card").forEach((card) => {
      const model = normalizeText(card.getAttribute("data-model") || "");
      card.style.display = !q || model.includes(q) ? "" : "none";
    });
  });
}

function bindServiceLogUI() {
  const openBtn = $("openServiceLogModal");
  const modal = $("serviceLogModal");
  if (!openBtn || !modal) return;

  openBtn.addEventListener("click", () => openModal("serviceLogModal"));

  $("saveServiceLogBtn")?.addEventListener("click", async () => {
    const user = auth.currentUser;
    if (!user) return showToast("Avval kirish qiling!", "error");

    const carId = $("logCarSelect")?.value || "";
    const type = $("logType")?.value || "boshqa";
    const km = Number.parseInt($("logKm")?.value || "", 10);
    const note = ($("logNote")?.value || "").trim();

    if (!carId || Number.isNaN(km) || km < 0) return showToast("Car va Km to‘g‘ri kiriting!", "error");
    ...
    try {
      ...
      showToast("Servis yozuvi saqlandi!", "success");
      closeModal("serviceLogModal");
      if ($("logKm")) $("logKm").value = "";
