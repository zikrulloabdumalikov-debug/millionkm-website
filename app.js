...
function openBrandModal(brand) {
  ...
  Object.entries(models).forEach(([model, data]) => {
    ...
    card.innerHTML = `
      <h4 class="model-title">${safeText(model)}</h4>
      <p class="model-desc">${safeText(data?.desc || "")}</p>

      <div class="model-badges">
        <span class="badge">1 martalik</span>
        <span class="badge">1 yillik</span>
      </div>

      <div class="model-limits">
        <span>Yosh: <b>${Number(data.maxAge)}</b> yil</span>
        <span>Probeg: <b>${Number(data.maxKm).toLocaleString()}</b> km</span>
      </div>

      <div class="model-price">
        <span class="price-label">1 martalik moy almashtrish:</span>
        <span class="price-value">${priceText}</span>
      </div>

      <div class="model-actions">
        <button class="btn detail-model-btn" type="button">Batafsil</button>
        <button class="btn btn-secondary order-model-btn" type="button">Buyurtma</button>
      </div>

      <div class="details-box hidden">
        <div class="details-grid">
          <div class="details-card">
            <h5>1 martalik xizmat</h5>
            <ul>
              <li>Avtomobil modeli bo‘yicha to‘g‘ri motor moyi tanlovi</li>
              <li>Dvigatel uchun aniq va yetarli hajmda motor moyi</li>
              <li>Moy filtrini almashtirish</li>
              <li>Havo filtrini almashtirish</li>
              <li>Salon filtrini almashtirish</li>
              <li>Moy darajasi va tizimni tekshirish</li>
              <li>Dvigatel ishga tushirilgach yakuniy nazorat</li>
              <li>Keyingi moy almashtirishgacha kafolat</li>
              <li>Keyingi servis uchun belgilash (stiker / eslatma)</li>
              <li>Bonus: avtomobil ustki qismini bepul avtomoyka</li>
              <li>Barcha xizmatlar tajribali mutaxassislar tomonidan bajariladi</li>
            </ul>
          </div>

          <div class="details-card">
            <h5>1 yillik xizmat</h5>
            <p class="details-note">Yiliga <b>3–12 marta</b> servis tanlaysiz:</p>
            <div class="year-count-buttons" role="group" aria-label="Yillik servis soni">
              ${Array.from({ length: 10 }, (_, i) => i + 3).map(n => `<button class="count-btn" type="button" data-count="${n}">${n} ta</button>`).join("")}
            </div>

            <div class="year-selected">
              Tanlangan: <b class="yearCountValue">0</b> ta
            </div>

            <div class="details-subtitle">Qo‘shimcha farqlari va bonuslar:</div>
            <ul>
              <li>Million km oila a’zolari safiga qo‘shilasiz</li>
              <li>Maxsus oilaviy futbolka sovg‘a</li>
              <li>Telegramdagi maxsus guruhdagi video darsliklar</li>
              <li>Doimiy foydalanilsa, 1 000 000 km gacha muammosiz yurish kafolati</li>
              <li>Shaxsiy avtomobilingiz holatini kuzatish va texnik maslahatlar</li>
              <li>Har bir xizmatdan keyin yakuniy xulosa va tavsiyalar</li>
              <li>Mutaxassislar bilan savol-javoblar</li>
              <li>Yil oxirida maxsus sertifikat</li>
              <li>Uzoq masofa yurganlar uchun maxsus belgi</li>
              <li>Shukurullohon Abdumalikov bilan 12 marta jonli efir</li>
            </ul>
          </div>
        </div>
      </div>
      ...
      <div class="order-form hidden">
        ...
        <input type="text" class="service-count-input hidden" placeholder="1 yillik: nechta xizmat (3–12) tanlang" readonly>

        <textarea class="problem-desc" placeholder="Qisqacha izoh (ixtiyoriy)"></textarea>
        <input type="tel" class="contact-phone" placeholder="Telefon raqamingiz">

        <button class="btn submit-order-btn" style="width: 100%; margin-top: 10px;" type="button">
          Buyurtma berish
        </button>
      </div>
    `;
    ...
    // 3..12 tanlash (1 yillik uchun)
    const detailsBtn = card.querySelector(".detail-model-btn");
    ...
    const serviceCountInput = card.querySelector(".service-count-input");
    const yearCountValueEl = card.querySelector(".yearCountValue");
    ...
    // Count buttons (3..12)
    card.querySelectorAll(".count-btn").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        e.stopPropagation();
        const n = Number.parseInt(btn.getAttribute("data-count") || "", 10);
        if (!n) return;
        ...
        if (yearCountValueEl) yearCountValueEl.textContent = String(n);
        if (serviceCountInput) serviceCountInput.value = `${n} ta`;
      });
    });

    function syncYearCountUI() {
      const isYear = (serviceTypeEl?.value || "") === "1year";
      serviceCountInput?.classList.toggle("hidden", !isYear);

      if (isYear) {
        serviceCountInput.value = selectedYearCount ? `${selectedYearCount} ta` : "";
      } else {
        serviceCountInput.value = "";
      }
    }

    serviceTypeEl?.addEventListener("change", (e) => {
      e.stopPropagation();
      syncYearCountUI();
    });
...
// Submit
    card.querySelector(".submit-order-btn")?.addEventListener("click", async (e) => {
      ...
      const yearCount = selectedYearCount;
      ...
      await sendTelegramMessage(
        ...
        (serviceType === "1year" ? `📅 Yiliga: ${yearCount} ta servis\n` : "") +
      ...
      // reset year count selection
      selectedYearCount = 0;
      card.querySelectorAll(".count-btn").forEach((b) => b.classList.remove("active"));
      if (yearCountValueEl) yearCountValueEl.textContent = "0";
      if (serviceCountInput) serviceCountInput.value = "";
   renderYearButtons(3);
    });
  });
